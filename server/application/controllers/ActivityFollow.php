...
class ActivityFollow extends CI_Controller {
    ...
    public function readTopicFollow() {        
        ...
        if ($result['loginState'] === Constants::S_AUTH) {
            ...
            if($user_id != 0)
            {
              $resSelf = DB::select('gActivityFollow', ['*'], ['topic_id' => $topic_id,'user_id' => $user_id]); 
            ...
            //$resOther = DB::select('gActivityFollow', ['*'], ['topic_id ' => $topic_id]); 

            $sql = "SELECT * FROM gActivityFollow WHERE topic_id=".$topic_id." and user_id<>".$user_id;
            
            $query = DB::raw($sql); 
            //$query = DB::raw("SELECT LAST_INSERT_ID() AS lastid");  
            $resOther = $query->fetchAll(PDO::FETCH_OBJ);

            if ($resOther === NULL) {
                $resOther = [];
            } 

            $res = array_merge($resSelf,$resOther);
