...
class ActivityFollowRemark extends CI_Controller {
	...
	public function add() {
        ...
        $remark = isset($_POST['remark']) ? $_POST['remark'] : ' no post remark';        
        $create_time = date('Y-m-d H:i:s');
        log_message('error',"userId".$user_id);    
        log_message('error',"follow_id".$follow_id);       
        log_message('error',"remark".$remark);        
        ...
        if ($result['loginState'] === Constants::S_AUTH) {
            DB::insert('gActivityFollowRemark', compact('follow_id','user_id','remark', 'create_time'));
           ...
           // $id = $this->fffuser->readAcitivity($result['userinfo']['openId']);
            ...
            $res = DB::row('gActivityFollowRemark', ['*'], compact('id')); 
    ...
    public function readFollowRemark() {        
        $follow_id = isset($_POST['followId']) ? $_POST['followId'] : ' no post followId';   
        log_message('error',"readFollowRemark follow_id".$follow_id);   

        $result = LoginService::check(); 

        if ($result['loginState'] === Constants::S_AUTH) {            
            $res = DB::select('gActivityFollowRemark', ['*'], ['follow_id' => $follow_id]); 
    ...
    //删除跟读
    public function del() {        
        $id = isset($_POST['remarkId']) ? $_POST['remarkId'] : ' no post remarkId';
        $user_id = isset($_POST['userId']) ? $_POST['userId'] : ' no post userId';
        ...
        if ($result['loginState'] === Constants::S_AUTH) {
            $res = DB::delete('gActivityFollowRemark',compact('id')); 
